...
//let createUser = require('./createUser.js')
...
MongoClient.connect("mongodb://localhost:27017", function (err, client) {
  ...
  db.collection('userData').createIndex({"email": 1}, function(err,result){
    console.log('???????????????', myData);
    if (err){
  ...
  var db = client.db('userData');
    db.collection('userData').findOne(myData, function (err, result) {
          if(err == null && result==null)
           ...
           db.collection('userData').insert('access_token',access_token);
         }
     });
   });
});
...
MongoClient.connect("mongodb://localhost:27017", function (err, client) {
  var db = client.db('userData');
  db.collection('userData').insertMany([myData],function(err,result){
    console.log(err)
      if(err){
